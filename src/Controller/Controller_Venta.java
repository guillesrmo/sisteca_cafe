...
public class Controller_Venta {
    ...
    public String table_produto_busqueda(JTable clasificacion_producto, String busqueda) {
        ...
        try {
            ...
            model.setColumnIdentifiers(new Object[]{"", "COD_BARRA", "DESCRIPCIÒN", "PRESENTACION", "CANT", "PRECIO_VENT", "", "", "", ""});
            if (rs.next()) {
            ...
            clasificacion_producto.getColumnModel().getColumn(3).setMaxWidth(80);
            clasificacion_producto.getColumnModel().getColumn(3).setMinWidth(80);
            clasificacion_producto.getColumnModel().getColumn(3).setPreferredWidth(80);
    ...
    public void table_venta(JTable venta) {
        try {
            ...
            String sSQL1 = "select ven.venta_id,\n"
                    ...
                    + "per.persona_ruc,per.persona_dni,per.persona_razon_social,ti_mo.moneda_descrip,\n"
                    + "ven.venta_serie,\n"
                    + "ven.venta_numero,\n"
                    + "ven.venta_fecha_emision,\n"
                    + "ven.venta_total from venta ven\n"
            ...
            model.setColumnIdentifiers(new Object[]{"", "COMPROBANTE", "DOCUMENTO", "RUC", "DNI", "RAZON SOCIAL", "MONEDA", "SERIE", "NUMERO", "FECHA", "TOTAL"});
            while (rs.next()) {
    ...
    public String Add_venta(Entity_Venta venta) {
        ...
        System.out.println("Controller.Controller_Venta.Add_venta()"+sent);
        try {
            cxn.conectardb().createStatement().executeUpdate(sent);
            cxn.desconectar();
            respuesta = "exito";
    ...
    public String Add_compra_producto_detalle(Entity_Venta_Producto detalle_compra) {
        ...
        System.out.println("Controller.Controller_Venta.Add_compra_producto_detalle()"+sent);
        try {
            cxn.conectardb().createStatement().executeUpdate(sent);
            cxn.desconectar();
            respuesta = "exito";
                ...
                public void keyPressed(KeyEvent evt) {
                    try {
                        if (evt.getKeyCode() == KeyEvent.VK_F1) {
                            Controller_Venta almacen = new Controller_Venta();
                        ...
                        if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
                            List<Entity_Producto_Presentacion> persona = new Controller_Venta().buscar_cod_barra_producto_presentacion(txt_cod_barra.getText());
                        ...
                        if (evt.isControlDown() && evt.getKeyCode() == KeyEvent.VK_L) {
                            try {
                                if (tabla.getValueAt(tabla.getSelectedRow() + getselection, 2).toString().length() != 0) {
                                    ...
                                    Controller_Venta almacen = new Controller_Venta();
                                    if (almacen.table_lote(View_Venta.tabla_lote, tabla.getValueAt(tabla.getSelectedRow() + getselection, 2).toString()).equals("ERROR")) {
                                        Function_ShowMessageDialog.ShowMessageDialogvalidacion(null, "BUSQUEDA NO ENCONTRADA");

                                    } else {
                                        almacen.table_lote(View_Venta.tabla_lote, tabla.getValueAt(tabla.getSelectedRow() + getselection, 2).toString());
                                        Dimension desktopSize = View_Escritorio.desktopPane.getSize();
                                        Dimension FrameSize = View_Venta.Dialog_lote.getSize();
                                        View_Venta.Dialog_lote.setLocation((desktopSize.width - FrameSize.width) / 2, (desktopSize.height - FrameSize.height) / 2);
                                        View_Venta.Dialog_lote.setVisible(true);
                                        tabla.editCellAt(tabla.getSelectedRow() + getselection, 10);
                                        Component aComp = tabla.getEditorComponent();
                                        aComp.requestFocus();
                                        View_Venta.tabla_lote.requestFocus();
                ...
                public void keyPressed(KeyEvent evt) {
                    ...
                    if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
                        if (txt_cantidad.getText().length() != 0) {
                            ...
                            Component aComp = tabla.getEditorComponent();
                            aComp.requestFocus();
                            View_Venta.btn_calcular.doClick();
                        ...
                        try {
                            if (txt_cantidad.getText().length() != 0) {
                                if (tabla.getSelectedRow() + getselection == tabla.getRowCount() - 1) {
                                    ...
                                    View.View_Venta.btn_agregar_fila.doClick();
...
"where venta_id='"+buscar_venta+"'";
        try {
            ...
            venta.add(new Entity_Venta(rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5),
                    ...
                    rs.getString(50),rs.getString(51),"","","","","",""));
            cxn.desconectar();
        } catch (SQLException ex) {
            Function_ShowMessageDialog.ShowMessageDialogwarning(null, ex);
        }
